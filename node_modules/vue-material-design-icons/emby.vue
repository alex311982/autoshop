<template>
  <div class="material-design-icon" id="emby-icon" role="img" aria-labelledby="emby-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="emby-icon-title">{{iconTitle}}</title>
      <path d="M11,2L6,7L7,8L2,13L7,18L8,17L13,22L18,17L17,16L22,11L17,6L16,7L11,2M10,8.5L16,12L10,15.5V8.5Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "emby-icon",
    data () {
      let iconTitle = this.title ? this.title : "Emby icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
