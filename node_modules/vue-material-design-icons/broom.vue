<template>
  <div class="material-design-icon" id="broom-icon" role="img" aria-labelledby="broom-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="broom-icon-title">{{iconTitle}}</title>
      <path d="M19.36,2.72L20.78,4.14L15.06,9.85C16.13,11.39 16.28,13.24 15.38,14.44L9.06,8.12C10.26,7.22 12.11,7.37 13.65,8.44L19.36,2.72M5.93,17.57C3.92,15.56 2.69,13.16 2.35,10.92L7.23,8.83L14.67,16.27L12.58,21.15C10.34,20.81 7.94,19.58 5.93,17.57Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "broom-icon",
    data () {
      let iconTitle = this.title ? this.title : "Broom icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
