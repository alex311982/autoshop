<template>
  <div class="material-design-icon" id="undo-icon" role="img" aria-labelledby="undo-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="undo-icon-title">{{iconTitle}}</title>
      <path d="M12.5,8C9.85,8 7.45,9 5.6,10.6L2,7V16H11L7.38,12.38C8.77,11.22 10.54,10.5 12.5,10.5C16.04,10.5 19.05,12.81 20.1,16L22.47,15.22C21.08,11.03 17.15,8 12.5,8Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "undo-icon",
    data () {
      let iconTitle = this.title ? this.title : "Undo icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
