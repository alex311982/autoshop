<template>
  <div class="material-design-icon" id="sleep-icon" role="img" aria-labelledby="sleep-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="sleep-icon-title">{{iconTitle}}</title>
      <path d="M23,12H17V10L20.39,6H17V4H23V6L19.62,10H23V12M15,16H9V14L12.39,10H9V8H15V10L11.62,14H15V16M7,20H1V18L4.39,14H1V12H7V14L3.62,18H7V20Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "sleep-icon",
    data () {
      let iconTitle = this.title ? this.title : "Sleep icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
