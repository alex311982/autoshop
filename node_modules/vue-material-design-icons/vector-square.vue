<template>
  <div class="material-design-icon" id="vector-square-icon" role="img" aria-labelledby="vector-square-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="vector-square-icon-title">{{iconTitle}}</title>
      <path d="M2,2H8V4H16V2H22V8H20V16H22V22H16V20H8V22H2V16H4V8H2V2M16,8V6H8V8H6V16H8V18H16V16H18V8H16M4,4V6H6V4H4M18,4V6H20V4H18M4,18V20H6V18H4M18,18V20H20V18H18Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "vector-square-icon",
    data () {
      let iconTitle = this.title ? this.title : "Vector square icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
