<template>
  <div class="material-design-icon" id="windows-icon" role="img" aria-labelledby="windows-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="windows-icon-title">{{iconTitle}}</title>
      <path d="M3,12V6.75L9,5.43V11.91L3,12M20,3V11.75L10,11.9V5.21L20,3M3,13L9,13.09V19.9L3,18.75V13M20,13.25V22L10,20.09V13.1L20,13.25Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "windows-icon",
    data () {
      let iconTitle = this.title ? this.title : "Windows icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
