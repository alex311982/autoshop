<template>
  <div class="material-design-icon" id="radiobox-marked-icon" role="img" aria-labelledby="radiobox-marked-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="radiobox-marked-icon-title">{{iconTitle}}</title>
      <path d="M12,20C7.58,20 4,16.42 4,12C4,7.58 7.58,4 12,4C16.42,4 20,7.58 20,12C20,16.42 16.42,20 12,20M12,2C6.48,2 2,6.48 2,12C2,17.52 6.48,22 12,22C17.52,22 22,17.52 22,12C22,6.48 17.52,2 12,2M12,7C9.24,7 7,9.24 7,12C7,14.76 9.24,17 12,17C14.76,17 17,14.76 17,12C17,9.24 14.76,7 12,7Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "radiobox-marked-icon",
    data () {
      let iconTitle = this.title ? this.title : "Radiobox marked icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
