<template>
  <div class="material-design-icon" id="drag-horizontal-icon" role="img" aria-labelledby="drag-horizontal-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="drag-horizontal-icon-title">{{iconTitle}}</title>
      <path d="M3,15V13H5V15H3M3,11V9H5V11H3M7,15V13H9V15H7M7,11V9H9V11H7M11,15V13H13V15H11M11,11V9H13V11H11M15,15V13H17V15H15M15,11V9H17V11H15M19,15V13H21V15H19M19,11V9H21V11H19Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "drag-horizontal-icon",
    data () {
      let iconTitle = this.title ? this.title : "Drag horizontal icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
