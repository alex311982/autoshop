<template>
  <div class="material-design-icon" id="marker-check-icon" role="img" aria-labelledby="marker-check-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="marker-check-icon-title">{{iconTitle}}</title>
      <path d="M10,16L5,11L6.41,9.58L10,13.17L17.59,5.58L19,7M19,1H5C3.89,1 3,1.89 3,3V15.93C3,16.62 3.35,17.23 3.88,17.59L12,23L20.11,17.59C20.64,17.23 21,16.62 21,15.93V3C21,1.89 20.1,1 19,1Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "marker-check-icon",
    data () {
      let iconTitle = this.title ? this.title : "Marker check icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
