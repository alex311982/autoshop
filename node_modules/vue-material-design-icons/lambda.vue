<template>
  <div class="material-design-icon" id="lambda-icon" role="img" aria-labelledby="lambda-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="lambda-icon-title">{{iconTitle}}</title>
      <path d="M6,20L10.16,7.91L9.34,6H8V4H10C10.42,4 10.78,4.26 10.93,4.63L16.66,18H18V20H16C15.57,20 15.21,19.73 15.07,19.36L11.33,10.65L8.12,20H6Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "lambda-icon",
    data () {
      let iconTitle = this.title ? this.title : "Lambda icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
