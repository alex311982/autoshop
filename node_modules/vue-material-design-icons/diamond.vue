<template>
  <div class="material-design-icon" id="diamond-icon" role="img" aria-labelledby="diamond-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="diamond-icon-title">{{iconTitle}}</title>
      <path d="M16,9H19L14,16M10,9H14L12,17M5,9H8L10,16M15,4H17L19,7H16M11,4H13L14,7H10M7,4H9L8,7H5M6,2L2,8L12,22L22,8L18,2H6Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "diamond-icon",
    data () {
      let iconTitle = this.title ? this.title : "Diamond icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
