<template>
  <div class="material-design-icon" id="panorama-fisheye-icon" role="img" aria-labelledby="panorama-fisheye-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="panorama-fisheye-icon-title">{{iconTitle}}</title>
      <path d="M12,20C7.59,20 4,16.41 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20,12C20,16.41 16.41,20 12,20M12,2C6.47,2 2,6.47 2,12C2,17.53 6.47,22 12,22C17.53,22 22,17.53 22,12C22,6.47 17.53,2 12,2Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "panorama-fisheye-icon",
    data () {
      let iconTitle = this.title ? this.title : "Panorama fisheye icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
