<template>
  <div class="material-design-icon" id="close-octagon-icon" role="img" aria-labelledby="close-octagon-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="close-octagon-icon-title">{{iconTitle}}</title>
      <path d="M8.27,3L3,8.27V15.73L8.27,21H15.73L21,15.73V8.27L15.73,3M8.41,7L12,10.59L15.59,7L17,8.41L13.41,12L17,15.59L15.59,17L12,13.41L8.41,17L7,15.59L10.59,12L7,8.41"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "close-octagon-icon",
    data () {
      let iconTitle = this.title ? this.title : "Close octagon icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
