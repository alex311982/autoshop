<template>
  <div class="material-design-icon" id="text-shadow-icon" role="img" aria-labelledby="text-shadow-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="text-shadow-icon-title">{{iconTitle}}</title>
      <path d="M3,3H16V6H11V18H8V6H3V3M12,7H14V9H12V7M15,7H17V9H15V7M18,7H20V9H18V7M12,10H14V12H12V10M12,13H14V15H12V13M12,16H14V18H12V16M12,19H14V21H12V19Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "text-shadow-icon",
    data () {
      let iconTitle = this.title ? this.title : "Text shadow icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
