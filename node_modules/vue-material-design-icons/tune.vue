<template>
  <div class="material-design-icon" id="tune-icon" role="img" aria-labelledby="tune-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="tune-icon-title">{{iconTitle}}</title>
      <path d="M3,17V19H9V17H3M3,5V7H13V5H3M13,21V19H21V17H13V15H11V21H13M7,9V11H3V13H7V15H9V9H7M21,13V11H11V13H21M15,9H17V7H21V5H17V3H15V9Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "tune-icon",
    data () {
      let iconTitle = this.title ? this.title : "Tune icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
