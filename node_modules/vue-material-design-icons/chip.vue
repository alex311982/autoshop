<template>
  <div class="material-design-icon" id="chip-icon" role="img" aria-labelledby="chip-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="chip-icon-title">{{iconTitle}}</title>
      <path d="M6,4H18V5H21V7H18V9H21V11H18V13H21V15H18V17H21V19H18V20H6V19H3V17H6V15H3V13H6V11H3V9H6V7H3V5H6V4M11,15V18H12V15H11M13,15V18H14V15H13M15,15V18H16V15H15Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "chip-icon",
    data () {
      let iconTitle = this.title ? this.title : "Chip icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
