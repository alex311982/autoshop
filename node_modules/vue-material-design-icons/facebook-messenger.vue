<template>
  <div class="material-design-icon" id="facebook-messenger-icon" role="img" aria-labelledby="facebook-messenger-icon-title">
    <svg class="material-design-icon__svg" width="24" height="24" viewBox="0 0 24 24">
      <title id="facebook-messenger-icon-title">{{iconTitle}}</title>
      <path d="M12,2C6.5,2 2,6.14 2,11.25C2,14.13 3.42,16.7 5.65,18.4L5.71,22L9.16,20.12L9.13,20.11C10.04,20.36 11,20.5 12,20.5C17.5,20.5 22,16.36 22,11.25C22,6.14 17.5,2 12,2M13.03,14.41L10.54,11.78L5.5,14.41L10.88,8.78L13.46,11.25L18.31,8.78L13.03,14.41Z"></path>
    </svg>
  </div>
</template>

<script>
  export default {
    name: "facebook-messenger-icon",
    data () {
      let iconTitle = this.title ? this.title : "Facebook messenger icon"

      return {
        iconTitle: iconTitle
      }
    },
    props: {
      title: {
        type: String
      }
    }
  }
</script>
